...
class InitData(
    ...
    override fun run(args: ApplicationArguments?) {
        ...
        listOf(
            ...
            ToDoList(id = Id(0), name = "Second list", items = setOf(), userId = userFoo.id, reminder = LocalDateTime.now().plusSeconds(30)),
            ToDoList(id = Id(0), name = "A list", items = setOf(ToDoListItem("First"), ToDoListItem("Second")), userId = userBar.id, reminder = null),
            ToDoList(id = Id(0), name = "Another list", items = setOf(ToDoListItem("Foo"), ToDoListItem("Bar")), userId = userBar.id, reminder = LocalDateTime.now().plusSeconds(42)),
